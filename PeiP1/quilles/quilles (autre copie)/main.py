...
tailleinitialex=0
tailleinitialey=0
listequillespossibles=[lit,halteres,caleçon,voiture,tomate,baguette,bonbon,boule,cafe,calculatrice,chat,chaussette,coca,couteau,dé,livreqd,Livrets,masque,pate,pokeball,PQ,masterpomme,tournevis,wumpus]
listequilles=[]
for _ in range(3):
    k=randint(8,15)
    ltemp=[]
    ...
    while len(quilles)>0:                               #boucle tant qu'il reste des quilles
        for tanim in range(10):
            ...
            if tailleinitialex!=window_width() or tailleinitialey!=window_height():
                ...
                afficherétagere(listequilles,quilles,manche)
