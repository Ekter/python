...
hgt:167cm eyr:2026"""
linput=input.split("\n\n")
listgood=["byr","iyr","eyr","hgt","hcl","ecl","pid"]
n=0
for i in range(1000):
    for i in linput:
        l=i.split()
        ...
        for li in l:
            lii=li.split(":")
            d[lii[0]]=lii[1]
        if "byr" in d and int(d["byr"])>=1920 and int(d["byr"])<=2002:
            ni+=1
        if "iyr" in d and int(d["iyr"])>=2010 and int(d["iyr"])<=2020:
            ni+=1
        if "eyr" in d and int(d["eyr"])>=2020 and int(d["eyr"])<=2030:
            ni+=1
        if "hgt" in d:
            s=d["hgt"]
            if s[-2:]=="cm":
                if int(s[:-2])>=150 and int(s[:-2])<=193:
                    ni+=1
            elif s[-2:]=="in":
                if int(s[:-2])>=59 and int(s[:-2])<=76:
                    ni+=1
        if "hcl" in d:
            s=d["hcl"]
            if s[0]=="#" and sum([s[i] in "0123456789abcdef" for i in range(1,len(s))])==6:
                ni+=1
        if "ecl" in d and d["ecl"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
            ni+=1
        if "pid" in d and sum([d["pid"][i] in "0123456789" for i in range(0,len(d["pid"]))])==9:
            ni+=1
        if ni==7:
            n+=1
